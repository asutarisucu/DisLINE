...
設定するのはDiscordのBotトークンです。アプリID・サーバーID・あなたのユーザーIDは起動時に判別します。
...
残したいトークは、たまに発言するか `/line open` を打てば、閉じられる順番が後ろになります。
...
DiscordのスタンプはLINEのスタンプIDに対応付けられないため、送れません。
...
Botが編集できるのは自分がWebhookで投稿したメッセージだけです。
あなたが投稿したままだと、送信取り消しのときに本文を差し替えられません。
ついでに自分の発言もLINEの表示名とアイコンで並びます。
...
どちらも削除ではなく差し替えです。前後のメッセージはそのまま残ります。
...
スラッシュコマンドは定義が変わったときだけ登録し直します。手作業は要りません。
...
再起動ポリシーが拾えるのは終了したプロセスだけです。
5分おきにLINEへ疎通確認し、3回続けて失敗したら自分から終了して再起動に任せます。
...
- 公式アカウントや広告も自動でチャンネル化されます。放っておけば自動クローズの対象になりますが、
...
- `npm audit` は間接依存の `thrift` にhigh 2件を報告します。thrift パッケージのHTTPサーバ・
  静的配信機能に対する指摘で、このBotはその機能を使っていません。
  linejsが依存のバージョンを固定しているため、DisLINE側では更新できません
...
LINEアカウントを掌握されると、メッセージの閲覧だけでなく、友だち全員へのなりすましまで影響が及びます。
被害の範囲は利用者数 × その人の人間関係になります。
預かるのをやめる以外に、この問題を消す方法はありません。
